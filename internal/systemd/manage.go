...
// ManagedUnit represents a unit file managed by hitch.
...
// List scans the unit directory for files with a "# Managed by hitch" header
...
// managed-by-hitch marker and optional source comment. Returns (unit, true, nil)
...
func parseManagedUnit(path string, system bool) (ManagedUnit, bool, error) {
	...
	for scanner.Scan() {
		...
		if strings.TrimSpace(line) == "# Managed by hitch" {
			managed = true
		}
